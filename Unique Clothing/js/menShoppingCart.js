...
let generateManShop = () => {
    return (shop.innerHTML = menItemsData
        .map((x) =>{
        let {id, name, price, image} = x;   
        let search = basket.find((x) => x.id === id ) || [];
        producthtml=`
            <div id=product-id-${id} class="item">
            <div class="top">
            <a href="manDetailProducts.html?id=${id}"> <!-- Add product ID to the URL -->
            <img src="${image}" alt="Pants" width="220" height="215">
        </a>
            `;
        ...
        producthtml+=`
            ...
            <div class="details">
            <h3>
                <a href="manDetailProducts.html?id=${id}">${name}</a> <!-- Add product ID to the URL -->
            </h3></br>
            <div class="price-quantity">
                    <h2>RM ${price}</h2>
                    <!-- <div class="buttons">
