...
let generateWomenShop = () => {
    return (shop.innerHTML = womenItemsData
        .map((x) =>{
        let {id, name, price, image} = x;   
        let search = basket.find((x) => x.id === id ) || [];
        producthtml=`
            <div id=product-id-${id} class="item">
            <div class="top">
            <a href="womanDetailProducts.html?id=${id}"> <!-- Add product ID to the URL -->
                <img src="${image}" alt="Pants" width="220" height="215">
            </a>
            `;
        ...
        producthtml+=`
            ...
            <div class="details">
                <h3>
                    <a href="womanDetailProducts.html?id=${id}">${name}</a> <!-- Add product ID to the URL -->
                </h3></br>
